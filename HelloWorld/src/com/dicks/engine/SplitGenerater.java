...
//		System.out.println(Spilt.getCombinations(10, 4));
		...
		Combination[] combinations = SplitGenerater.getCombinations(5,2, items);
...
//		}
	...
	public static String getCombinations(int n,int m){
		...
		return sb.toString().substring(Integer.valueOf(indexResult[0]),Integer.valueOf(indexResult[1]));
	...
	private static String split(Integer[] inputArray,int n){
		...
		if("".equals(checkResult)) return null;
		...
		for(int i = 0; i < result2Array.length ; i++){
			...
			for(int j = 0; j < strs.length ; j++){
				...
				for(int k =0; k< strs.length ; k++){
					if(k!=j) sb.append(strs[k]).append("/");
				}
	...
	private static void put(String key, String value){
		if(!combinations.containsKey(key)) combinations.put(key, value);
	}
